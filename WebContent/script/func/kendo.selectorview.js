...
(function() {
	...
	KendoSelectorView.prototype.getInnerHTML = function() {
		return "<table>" + 
			...
			"<tr><td><button type='button' class='" + this.assignButtonClass + "' id='" + this.assignButtonId + "'>" + this.assignButtonTitle + "</button></td>" + 
			"<td><button type='button' class='" + this.unassignButtonClass + "' id='" + this.unassignButtonId + "'>" + this.unassignButtonTitle + "</button></td></tr>" +
			"</table>";
	};
	...
	KendoSelectorView.prototype.show = function() {
		...
		this.src.show(this.srcAttr, this.enableDblClickMove, this.dragRowMove);
		this.dst.show(this.dstAttr, this.enableDblClickMove, this.dragRowMove);
	 ...
	 * @param attr
	 */
	KendoTable.prototype.show = function(attr, dblClickRowMove, dragRowMove) {
		var _this = this;
		this.table.kendoGrid({
			...
			sortable : attr.sortable ? attr.sortable : false,
		...
		// 绑定行双击事件
		if (dblClickRowMove) {
			this.table.find(".k-selectable").delegate("tr", "dblclick", function() {
		...
		// 绑定行拖动事件
		if (dragRowMove) {
			this.table.find("table.k-selectable").delegate("tr", "mouseover", function() {
				if (!$(this).prop("draggable")) {
					$(this).prop("draggable", true);
				}
			}).delegate("tr", "dragstart", function() {
				dragInfo.srcTable = _this;
				var rowId = $(this).find("td:eq(0) input:checkbox[name='rowCheck']").attr("rowid");
				dragInfo.dataId = rowId;
				$(this).addClass(attr.rowCheckedClass);
			});
			
			this.table.bind("dragenter", function() {
				if (dragInfo.srcTable) {
					dragInfo.dstTable = _this;
				}
			}).bind("dragover", function(e) {
				e.preventDefault();
			}).bind("dragend", function() {
				if (dragInfo.srcTable && dragInfo.dstTable) {
					moveData(dragInfo.srcTable.getDataSource(), dragInfo.dstTable.getDataSource(), dragInfo.dataId);
					dragInfo = {};
				}
			});
		}
